...
module CTRL_UNIT(   
    ...
    reg [7:0] wdata_tx_reg, wdata_tx_next;
    reg [1:0] sw_mode_reg;

    assign wr_tx = wr_tx_reg;
    assign wdata_tx = wdata_tx_reg;
    ...
    always @(posedge clk, posedge reset) begin
        if(reset) begin
            ...
            wdata_tx_reg <= wdata_tx_next;
            wr_tx_reg <= wr_tx_next;
            sw_mode_reg <= sw_mode[4:3];
        end
    end

    always @(*) begin
        wdata_tx_next = wdata_tx_reg;
        wr_tx_next = wr_tx_reg;
        if(sw_mode_reg == 2'b00 || sw_mode_reg == 2'b01) begin
            wdata_tx_next = rdata_rx;
            wr_tx_next = empty_rx_b;
        end
        else if(sw_mode_reg == 2'b10) begin
            wdata_tx_next = data_HCSR04_tx;
            wr_tx_next = wr_tx_HCSR04;
        end
        else if(sw_mode_reg == 2'b11) begin
            wdata_tx_next = data_DHT11_tx;
            wr_tx_next = wr_tx_DHT11;
        end
    end
endmodule
