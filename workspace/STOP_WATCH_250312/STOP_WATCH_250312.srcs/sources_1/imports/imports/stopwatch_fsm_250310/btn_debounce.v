...
module btn_debounce(
    ...
    // next
    always @(i_btn, r_1khz) begin   // evnet i_btn, r_1khz
        // q_reg 현재의 상위 7비트를 
        // 다음 shift register의 하위 7비트(6:1)에 넣고, 
        // 최상에는 i_btn을 넣어라
        q_next = {i_btn, q_reg[7:1]}; // 8shift 동작 설명 // 끝에 거 왜 하나 버리징
    end

    // 8 input AND
    assign btn_debounce = &q_reg;

    // edge_detector , 100MHz // 엣지 검출
    // F.F이 하나 필요함
    always @ (posedge clk, posedge reset) begin
        if (reset) begin
            edge_detect <= 0;
